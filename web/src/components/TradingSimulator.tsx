...
export function TradingSimulator({ mode, symbol, ticker, candles, interval, onInterval, forecast, microstructure, futures, portfolio, orders, submitting, onSubmit, onKillSwitch }: TradingSimulatorProps) {
  ...
  return (
    <main className="exchange-page">
      ...
      <div className="exchange-workspace-grid">
        ...
        <form className="order-entry-panel exchange-ticket" onSubmit={submit}>
          ...
          <dl className="ticket-costs">
            <div><dt>Notional</dt><dd>{formatCompactUsd(costs.notional)}</dd></div>
            <div><dt>Fee + slippage / round trip</dt><dd>{formatCompactUsd(costs.roundTripCost)}</dd></div>
            ...
            {mode === "futures" ? <div><dt>Approx. liquidation</dt><dd className="negative">{formatPrice(costs.liquidation)}</dd></div> : null}
          </dl>
      ...
      <p className="exchange-caveat">Paper simulation only. Fees vary by Binance tier, maker/taker status, region, discounts and instrument. Futures liquidation excludes funding and maintenance-margin tiers.</p>
    </main>
  );
}
