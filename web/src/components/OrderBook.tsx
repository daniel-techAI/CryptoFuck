...
function BookRow({ level, side, maxTotal }: { level: OrderBookLevel; side: "ask" | "bid"; maxTotal: number }) {
  const style = { "--depth": `${maxTotal ? Math.min(100, level.total / maxTotal * 100) : 0}%` } as CSSProperties;
  return <div className={`book-row ${side}`} style={style}><strong>{formatPrice(level.price)}</strong><span>{level.quantity.toLocaleString("en-US", { maximumFractionDigits: 5 })}</span><span>{formatCompactUsd(level.total)}</span></div>;
}
...
export function OrderBook({ microstructure, ticker }: { microstructure: MarketMicrostructure; ticker: LiveTicker }) {
  ...
  return (
    <section className="order-book-panel" aria-label="Live Binance order book">
      ...
      <div className="book-head"><span>Price (USDT)</span><span>Amount</span><span>Total</span></div>
      <div className="book-levels asks">{asks.map((level) => <BookRow key={`a-${level.price}`} level={level} side="ask" maxTotal={maxTotal} />)}</div>
      ...
      <div className="book-levels bids">{bids.map((level) => <BookRow key={`b-${level.price}`} level={level} side="bid" maxTotal={maxTotal} />)}</div>
